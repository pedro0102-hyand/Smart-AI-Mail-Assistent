...
const lightColors = {
  pageBg: "#F8F7F4", card: "white", border: "transparent",
  shadow: "0 1px 3px rgba(0,0,0,0.05)", title: "#1A1A2E",
  text: "#374151", muted: "#9CA3AF", inputBg: "#FAFAFA", replyToBg: "#F9FAFB",
};

const darkColors = {
  pageBg: "#0F0F14", card: "#16161F", border: "#2A2A3A",
  shadow: "none", title: "#E5E7EB",
  text: "#D1D5DB", muted: "#6B7280", inputBg: "#1E1E2E", replyToBg: "#1E1E2E",
};
...
export default function EmailDetailPage() {
  ...
  const padding = isMobile ? "16px" : "32px 40px";
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: c.pageBg, fontFamily: "system-ui, sans-serif", transition: "background 0.3s" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <main style={{ flex: 1, padding, maxWidth: "860px", width: "100%" }}>
          {/* Back button */}
          <button
          ...
          {loading ? <LoadingView /> : !email ? (
            <div style={{ padding: "60px", textAlign: "center", color: c.muted }}>E-mail não encontrado.</div>
          ) : (
            <>
              {/* Header card */}
              <div style={{ background: c.card, borderRadius: "16px", padding: isMobile ? "18px" : "28px", marginBottom: "16px", boxShadow: c.shadow, border: `1px solid ${c.border}` }}>
                {/* Subject + Analyze button */}
                <div style={{
                  display: "flex",
                  ...
                  <h1 style={{ fontSize: isMobile ? "16px" : "20px", fontWeight: "700", color: c.title, margin: 0, fontFamily: "Georgia, serif", lineHeight: "1.3", flex: 1 }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <div style={{
                      ...
                      fontSize: isMobile ? "13px" : "15px", fontWeight: "700", flexShrink: 0,
                    }}>
                      {(email.sender || "?")[0].toUpperCase()}
                    </div>
                    <div>
                      <div style={{ fontSize: "14px", fontWeight: "600", color: c.title }}>
                  ...
                  <div style={{ fontSize: "11px", color: c.muted }}>
                    {isMobile ? formatDateShort(email.date) : formatDateFull(email.date)}
                  </div>
                </div>
                ...
                {email.analysis && (
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    {email.analysis.category && (() => {
                      ...
                      const cs = CATS[cat] || CATS.outro;
                      return (
                        <span style={{ fontSize: "12px", fontWeight: "600", padding: "4px 10px", borderRadius: "8px", background: cs.bg, color: cs.text, border: `1.5px solid ${cs.border}` }}>
                          {cat}
                        </span>
                      );
                    })()}
                    {email.analysis.urgency && (() => {
                      const uc = URGENCY_CONFIG[email.analysis.urgency];
                      if (!uc) return null;
                      return (
                        <span style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", fontWeight: "600", padding: "4px 10px", borderRadius: "8px", background: dark ? uc.bgDark : uc.bg, color: uc.color }}>
                          <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: uc.color, flexShrink: 0 }} />
                          {uc.label}
                        </span>
                      );
                    })()}
              ...
              {email.analysis && (
                <div style={{
                  ...
                  borderRadius: "16px", padding: isMobile ? "18px" : "24px", marginBottom: "16px",
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
                    <div style={{ width: "26px", height: "26px", background: "linear-gradient(135deg, #6366F1, #8B5CF6)", borderRadius: "7px", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "13px", flexShrink: 0 }}>✦</div>
                    <span style={{ fontSize: "14px", fontWeight: "700", color: dark ? "#C4B5FD" : "#4C1D95" }}>Análise da IA</span>
                  </div>
                  <div style={{ fontSize: "11px", fontWeight: "700", color: dark ? "#A78BFA" : "#7C3AED", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "8px" }}>Resumo</div>
              ...
              <div style={{ background: c.card, borderRadius: "16px", padding: isMobile ? "18px" : "28px", marginBottom: "16px", boxShadow: c.shadow, border: `1px solid ${c.border}` }}>
                ...
                {email.body ? (
                  isHtml(email.body) ? (
                    <div
                      style={{ fontSize: "14px", color: c.text, lineHeight: "1.8", wordBreak: "break-word", maxWidth: "100%", overflow: "hidden" }}
              ...
              {/* Reply section */}
              <div style={{ background: c.card, borderRadius: "16px", padding: isMobile ? "18px" : "28px", boxShadow: c.shadow, border: `1px solid ${c.border}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px", flexWrap: "wrap" }}>
                  ...
                  {email.analysis?.suggested_reply && !replyEdited && (
                    <span style={{ fontSize: "10px", fontWeight: "700", background: "linear-gradient(135deg, #6366F1, #8B5CF6)", color: "white", padding: "2px 7px", borderRadius: "6px", letterSpacing: "0.5px" }}>IA</span>
                  )}
                  {replyEdited && (
                    <span style={{ fontSize: "10px", fontWeight: "600", background: "#FEF3C7", color: "#D97706", padding: "2px 7px", borderRadius: "6px" }}>editado</span>
                  )}
                </div>

                <div style={{ fontSize: "12px", color: c.text, marginBottom: "14px", padding: "8px 12px", background: c.replyToBg, borderRadius: "8px", border: `1px solid ${c.border}`, wordBreak: "break-word" }}>
                ...
                <textarea
                  style={{
                    ...
                    fontSize: "14px", color: c.text, lineHeight: "1.7", resize: "vertical",
                    outline: "none", fontFamily: "system-ui, sans-serif", background: c.inputBg,
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "12px", flexWrap: "wrap" }}>
                  <button
                    style={{ padding: "9px 14px", border: `1.5px solid ${c.border}`, borderRadius: "10px", background: c.card, fontSize: "13px", fontWeight: "500", color: c.muted, cursor: "pointer" }}
